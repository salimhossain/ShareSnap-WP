...
class ShareSnap_WP_Admin {
	...
	public function render_meta_box( $post ) {
		...
		$url = add_query_arg(
		    array(
		        'page' => 'sharesnap-wp',
		        'p'    => $post_id,
		        '_wpnonce' => wp_create_nonce( 'sharesnap_wp_action' ),
		    ),
		    admin_url( 'admin.php' )
		);
		?>
		<div class="sharesnap-meta-box-wrapper">
			<div>
				<div class="post-link">
					<a id="sharesnap-title" href="<?php echo esc_url( $url ); ?>" target="_blank" class="swp-d-flex swp-align-items-center goto-post" style="display: flex;font-size: 16px;text-decoration: none;">
	...
	public function display_admin_page() {
		...
		if (isset( $_GET['_wpnonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_GET['_wpnonce'] ) ), 'sharesnap_wp_action' ))
		{
